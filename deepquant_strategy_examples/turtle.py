...
def handle_bar(context, bar_dict):
    ...
    high_price = history_bars(context.s, context.open_observe_time + 1, '1d', 'high_price')
    low_price_for_atr = history_bars(context.s, context.open_observe_time + 1, '1d', 'low_price')
    low_price_for_extreme = history_bars(context.s, context.close_observe_time + 1, '1d', 'low_price')
    close_price = history_bars(context.s, context.open_observe_time + 2, '1d', 'close_price')
    ...
    if context.trading_signal != 'start':
        ...
        context.max_add = bar_dict[context.s].last_price
    ...
    if (cur_position > 0 and
            bar_dict[context.s].last_price < get_stop_price(context.first_open_price, context.units_hold, atr)):
        context.trading_signal = 'stop'
    else:
        if cur_position > 0 and bar_dict[context.s].last_price < minn:
            context.trading_signal = 'exit'
        else:
            if (bar_dict[context.s].last_price > context.max_add and context.units_hold != 0 and
                    context.units_hold < context.units_hold_max and
                    available_cash > bar_dict[context.s].last_price*context.unit):
                context.trading_signal = 'entry_add'
            else:
                if bar_dict[context.s].last_price > maxx and context.units_hold == 0:
                    context.max_add = bar_dict[context.s].last_price
    ...
    if (context.trading_signal != context.pre_trading_signal or
        ...
        if context.trading_signal == 'entry':
            context.quantity = context.unit
            if available_cash > bar_dict[context.s].last_price*context.quantity:
                order_shares(context.s, context.quantity)
                context.first_open_price = bar_dict[context.s].last_price
